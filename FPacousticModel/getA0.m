...
list_n=vib_p.n;
for j=1:n
    molj=gasT(gasT.Name==molList(j),:);
    modj=vib_p(j,:);


    m=length(molList);
    for l=1:m
        moll=gasT(gasT.Name==molList(l),:);
        tau_vib_j(l)=vibrationTime(molj,modj,moll,vib_p(l,:),T);
    end
    nj=modj.n;
    ...
    list_nj=list_n;
    list_nj(j)=[];
    q(j)=1/totalTranslationTime(molj,modj,model)+sum(other_tau_vib.^-1.*(1-exp(-h*nj/(kb*T)))./(1-exp(-h*list_nj'/kb/T)).*(1-list_nj/nj)');
    ...
    for k=1:n
        ...
        if j==k
            ...
            list_nj=list_n;
            list_nj(j)=[];
            A(j,k)=1/totalTranslationTime(molj,modj,model)+sum(other_tau_vib.^-1.*(1-exp(-h*nj/(kb*T)))./(1-exp(-h*list_nj'/kb/T)));
        else
